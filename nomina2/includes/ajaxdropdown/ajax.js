...
Ajax.Response = function ()
{
	...
	{		
		...
		document.getElementById('selSubCat').options[0] = new Option("--Selecione--","0");
		
		var	response = Ajax.request.responseXML.documentElement;
		var _data = response.getElementsByTagName('category');
		if(_data.length == 0)
		{
			document.getElementById('selSubCat').options[0] = new Option("No existe una subcategoria disponible",'0');	
		}
		var i
...
Ajax.CheckReadyState = function(obj)
{
	if(obj.readyState < 4) {		
		...
		document.getElementById('loading').innerHTML = "<table border=0 cellpadding=0 cellspacing=1 width=160 bgcolor=gray><tr><td align=center class=loading height=45 bgcolor=#ffffff><img src='snake_transparent.gif'/>&nbsp&nbspCargando..</td></tr></table>";  
	}
	...
	if(obj.readyState == 4)
	{
		if(obj.status == 200)
		{
			document.getElementById('loading').innerHTML = "<table border=0 cellpadding=0 cellspacing=1 width=160 bgcolor=gray><tr><td align=center class=loaded height=45 bgcolor=#ffffff>Datos cargados exitosamente..</td></tr></table>";
