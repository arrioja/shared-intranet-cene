...
{
$res=mysql_query("SELECT cod_plan_e_dir FROM gestion_plan_e_o_dir WHERE cod_plan_o_dir=$_GET[seleccionado]",$link)or die();
$row=mysql_fetch_array($res);
$seleccionado=$row["cod_plan_e_dir"]; 

$result=mysql_query("SELECT * FROM gestion_obj_estrategicos_direcciones AS a inner JOIN gestion_obje_objo_dir AS b ON a.codigo = b.cod_obj_e_dir WHERE b.cod_obj_o_dir=$_GET[codigo] and a.cod_plan_e_dir='$seleccionado'",$link)or die();
...
{
$result=mysql_query("SELECT * FROM gestion_fases AS a inner JOIN gestion_relacion_actividades_fases AS b ON a.id = b.precedida WHERE b.fase=$_GET[codigo] and a.id<>$_GET[codigo] and a.cod_actividad= $_GET[seleccionado]",$link)or die();
